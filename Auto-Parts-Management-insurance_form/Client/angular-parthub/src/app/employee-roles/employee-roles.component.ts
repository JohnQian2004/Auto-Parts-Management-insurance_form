...
export class EmployeeRolesComponent implements OnInit, AfterViewInit {
  ...
  tatoalPrecentage: any = 0;

  getTotalPrecentage(): any {

    var tatoalPrecentage = 0;
    for (let role of this.employeeRoles) {
      tatoalPrecentage += role.precentage;
    }
    this.tatoalPrecentage = tatoalPrecentage;
    return tatoalPrecentage;
  }
  createEmployeeRole(employeeRole: EmployeeRole): void {
    ...
    // if (employeeRole.precentage == null || employeeRole.precentage == 0) {
    //   this.messageEmployeeRole = "Employee Role Precentage is required";
    //   return;
    // }

    if (employeeRole.precentage > 0 && employeeRole.precentage > 100) {
      this.messageEmployeeRole = "Employee Role Precentage cannot be more than 100";
      return;
    }

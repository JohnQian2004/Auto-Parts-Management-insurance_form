...
export class PaymentStatusComponent implements OnInit, AfterViewInit {
  ...
  createPaymentStatus(paymentStatus: PaymentStatus): void {
    ...
    if (paymentStatus.name != null && paymentStatus.name != '' && paymentStatus.name.length < 2) {

      this.messagePaymentStatus = "Payment Status Name is too short";
      return;
    }

    if (paymentStatus.comments != null && paymentStatus.comments != '' && paymentStatus.comments.length < 2) {

      this.messagePaymentStatus = "Pament Status Comments are too short";
      return;
    }

    if (paymentStatus.name != null && paymentStatus.name != '' && paymentStatus.name.length > 255) {

      this.messagePaymentStatus = "Payment Status Name is too long";
      return;
    }

    if (paymentStatus.comments != null && paymentStatus.comments != '' && paymentStatus.comments.length > 255) {

      this.messagePaymentStatus = "Pament Status Comments are too long";
      return;
    }

    var newEntry: boolean = false;
    if (paymentStatus.id == null || paymentStatus.id == 0)
      newEntry = true;
    ...
    this.paymentStatusService.createPaymentStatus(this.currentUser.id, paymentStatus).subscribe({
      next: result => {
        ...
        if (newEntry == true)
          this.messagePaymentStatus = "Successfully Created";
        else

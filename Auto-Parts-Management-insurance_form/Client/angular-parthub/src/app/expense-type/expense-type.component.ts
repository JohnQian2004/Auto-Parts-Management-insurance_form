...
export class ExpenseTypeComponent implements OnInit, AfterViewInit {
  ...
  deleteExpenseType(): void {
    ...
    this.expenseTypeService.deleteExpenseType(this.expenseType.id).subscribe({
      next: result => {

        console.log(result);
        this.getCompanyExpenseType(this.company.id);
  ...
  createExpenseType(expenseType: ExpenseType): void {

    expenseType.companyId = this.company.id;

    if (expenseType.name == null || expenseType.name == '') {
      this.messageVehicleExpenseType = "Expense TypeName is required";
      return;
    }

    if (expenseType.name != null && expenseType.name != '' && expenseType.name.length < 2) {

      this.messageVehicleExpenseType = "Expense TypeName is too short";
      return;
    }

    if (expenseType.comments != null && expenseType.comments != '' && expenseType.comments.length < 2) {

      this.messageVehicleExpenseType = "Expense TypeComments are too short";
      return;
    }

    if (expenseType.name != null && expenseType.name != '' && expenseType.name.length > 255) {

      this.messageVehicleExpenseType = "Expense TypeName is too long";
      return;
    }

    if (expenseType.comments != null && expenseType.comments != '' && expenseType.comments.length > 255) {

      this.messageVehicleExpenseType = "Expense TypeComments are too long";
      return;
    }

    var newEntry: boolean = false;
    if (expenseType.id == null || expenseType.id == 0)
      newEntry = true;


    this.expenseTypeService.createExpenseType(this.currentUserUser.id, expenseType).subscribe({
      next: result => {
        this.expenseType = result;

        if (newEntry == true)
          this.messageVehicleExpenseType = "Successfully Created";
        else
          this.messageVehicleExpenseType = "Successfully Updated";
        this.getCompanyExpenseType(this.company.id);
      }
    })
  }
  ...
  messageVehicleExpenseType: any = "";

  addNewExpenseType(): void {
    this.expenseType = new ExpenseType();
    this.messageVehicleExpenseType = "";
  }
  ...
  getExpenseDetail(expenseType: ExpenseType, index: any): void {

...
export class BannerComponent implements OnInit, AfterViewInit {
  ...
  addNewServiceType(): void {
    ...
    this.messageServiceType = "";
  }

  messageBanner: any = null;
  ...
  createBanner(banner: Banner): void {
    ...
    //banner.companyId = this.company.id;

    if (banner.title == null || banner.title == '') {
      this.messageBanner = "Title is required";
      return;
    }

    if (banner.title != null && banner.title != '' && banner.title.length < 2) {

      this.messageBanner = "Title is too short";
      return;
    }


    if (banner.title != null && banner.title != '' && banner.title.length > 255) {

      this.messageBanner = "Title is too long";
      return;
    }

    if (banner.content != null && banner.content != '' && banner.content.length > 2000) {

      this.messageBanner = "Content are too long";
      return;
    }

    if (banner.content != null && banner.content != '' && banner.content.length < 2) {

      this.messageBanner = "Content are too shot";
      return;
    }

    // var hasitDup: boolean = false;
    // if (this.banners != null && this.banners.length > 0) {
    //   for (let banner1 of this.banners) {
    //     if (banner.id != banner1.id && banner.title.toUpperCase() == banner1.title?.toUpperCase()) {
    //       // banner1 = this.banner;
    //       hasitDup = true;
    //     }
    //   }
    //   if (hasitDup == true) {
    //     this.messageBanner = "[" + banner.title + "] is already provisioned, please try another one ";
    ...
    this.bannerService.createUpdateBanner(this.currentUser.id, banner).subscribe({
      next: result => {
        this.banner = result;
        if (newEntry == true)
          this.messageBanner = "Successfully Created";
        else
          this.messageBanner = "Successfully Updated";

        var hasIt = false;
        if (this.banners != null && this.banners.length > 0) {
          for (let banner1 of this.banners) {
            if (banner.id == banner1.id) {
              banner1 = this.banner;
              hasIt = true;
            }
          }
  ...
  createServiceType(serviceType: ServiceType): void {

    //serviceType.companyId = this.company.id;

    if (serviceType.name == null || serviceType.name == '') {
      this.messageServiceType = "Service Type Name is required Comment is optional";
      return;
    }

    if (serviceType.name != null && serviceType.name != '' && serviceType.name.length < 2) {

      this.messageServiceType = "Service Type Name is too short";
      return;
    }


    if (serviceType.name != null && serviceType.name != '' && serviceType.name.length > 255) {

      this.messageServiceType = "Service Type Name is too long";
      return;
    }

    if (serviceType.comments != null && serviceType.comments != '' && serviceType.comments.length > 255) {

      this.messageServiceType = "Service Type Comments are too long";
      return;
    }

    if (serviceType.comments != null && serviceType.comments != '' && serviceType.comments.length < 2) {

      this.messageServiceType = "Service Type Comments are too shot";
      return;
    }

    var hasitDup: boolean = false;
    if (this.serviceTypes != null && this.serviceTypes.length > 0) {
      for (let serviceType1 of this.serviceTypes) {
        if (serviceType.id != serviceType1.id && serviceType.name.toUpperCase() == serviceType1.name?.toUpperCase()) {
          // serviceType1 = this.serviceType;
          hasitDup = true;
        }
      }
      if (hasitDup == true) {
        this.messageServiceType = "[" + serviceType.name + "] is already provisioned, please try another one ";
    ...
    var newEntry: boolean = false;
    if (serviceType.id == null || serviceType.id == 0)
      newEntry = true;

    this.serviceTypeService.createUpdateServiceType(this.currentUser.id, serviceType).subscribe({
      next: result => {
        this.serviceType = result;
        if (newEntry == true)
          this.messageServiceType = "Successfully Created";
        else
          this.messageServiceType = "Successfully Updated";

        var hasIt = false;
        if (this.serviceTypes != null && this.serviceTypes.length > 0) {
          for (let serviceType1 of this.serviceTypes) {
            if (serviceType.id == serviceType1.id) {
              serviceType1 = this.serviceType;
              hasIt = true;
            }
          }
  ...
  createEmployeeRole(employeeRole: EmployeeRole): void {
    ...
    if (employeeRole.name == null || employeeRole.name == '') {
      this.messageEmployeeRole = "Employee Role Name is required Comment is optional";
      return;
    }

    if (employeeRole.name != null && employeeRole.name != '' && employeeRole.name.length < 2) {

      this.messageEmployeeRole = "Employee Role Name is too short";
      return;
    }


    if (employeeRole.name != null && employeeRole.name != '' && employeeRole.name.length > 255) {

      this.messageEmployeeRole = "Employee Role Name is too long";
      return;
    }

    if (employeeRole.comments != null && employeeRole.comments != '' && employeeRole.comments.length > 255) {

      this.messageEmployeeRole = "Employee Role Comments are too long";
      return;
    }

    if (employeeRole.comments != null && employeeRole.comments != '' && employeeRole.comments.length < 2) {

      this.messageEmployeeRole = "Employee Role Comments are too shot";
      return;
    }


    var newEntry: boolean = false;
    if (employeeRole.id == null || employeeRole.id == 0)
      newEntry = true;
    ...
    this.employeeRoleService.createEmployeeRole(this.currentUser.id, employeeRole).subscribe({
      next: result => {
        ...
        if (newEntry == true)
          this.messageEmployeeRole = "Successfully Created";
        else
  ...
  deleteServiceType(): void {
    ...
    this.serviceTypeService.deleteServiceType(this.serviceType.id).subscribe({
      next: result => {
  ...
  deleteBanner(): void {
    ...
    this.bannerService.deleteBanner(this.banner.id).subscribe({
      next: result => {
  ...
  getServiceTypeDetail(serviceType: any, index: any): void {
  ...
  setActive(banner: Banner): void {

    console.log("setActive");

    const customTitle = 'Set Banner to be active/inactive';
    const message = 'Are you sure to set this banner to [' + banner.active + '] ?';
    ...
    this.confirmationService.confirm(message, customTitle, buttonType, (confirmed: boolean) => {
      if (confirmed == undefined) {
        ...
        this.bannerService.setActive(this.currentUser.id, banner).subscribe({
          next: result => {
            banner = result;
            this.banner = result;
            this.getAllBanners();
  ...
  getBannerDetail(banner: any, index: any): void {

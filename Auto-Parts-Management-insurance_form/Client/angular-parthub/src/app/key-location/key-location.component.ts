...
import { KeyLocationService } from '../_services/key.location.service';
declare var bootstrap: any;
@Component({
  selector: 'app-key-location',
  templateUrl: './key-location.component.html',
  styleUrls: ['./key-location.component.css']
})
export class KeyLocationComponent implements OnInit, AfterViewInit {
  ...
  locations: KeyLocation[] = new Array();
  location: KeyLocation = new KeyLocation();
  ...
  constructor(
    ...
    private locationService: KeyLocationService,
  ...
  createLocation(location: Location): void {
    ...
    if (location.name == null || location.name == '') {
      this.messageVehicleLocation = "Location Name is required";
      return;
    }

    if (location.name != null && location.name != '' && location.name.length > 255) {

      this.messageVehicleLocation = "Location Name is too long";
      return;
    }

    if (location.name != null && location.name != '' && location.name.length < 2) {

      this.messageVehicleLocation = "Location Name is too short";
      return;
    }

    if (location.comments != null && location.comments != '' && location.comments.length > 255) {

      this.messageVehicleLocation = "Location Comments are too long";
      return;
    }

    if (location.comments != null && location.comments != '' && location.comments.length < 2) {

      this.messageVehicleLocation = "Location Comments are too short";
      return;
    }
    ...
    this.locationService.createLocation(this.currentUser.id, location).subscribe({
      next: result => {
        this.status = result;
        if (newEntry == true)
          this.messageVehicleLocation = "Successfully Created";
        else
    ...
    this.settingService.getSetting(companyId).subscribe({
      next: result => {
        if (result) {
          ...
          this.locations = this.setting.keyLocations;

...
@Component({
  selector: 'app-inshop3',
  templateUrl: './inshop3.component.html',
  styleUrls: ['./inshop3.component.css']
})
export class Inshop3Component implements OnInit, AfterViewInit {
    ...
    this.userService.getUserById(userId).subscribe({
      next: result => {
        ...
        if (this.user.companyId != 0) {
          ...
          this.searchVehicle(10, 0, this.pageSize);
  ...
  getJobRequestTypeOverview(companyId: any): void {
    ...
    this.vehicleService.getJobRequestTypeOverviewSpecial(companyId).subscribe({
      next: result => {
        console.log(result);
  ...
  columnDefinition = ['id', 'year', 'make', 'model', 'color', 'jobRequestType', 'status', 'serviceManager', 'assignedTo', 'daysInShop', 'comments'];

  inColumnDefinitions(columnName: any): boolean {

    for (let i = 0; i < this.columnDefinition.length; i++) {
      if (this.columnDefinition[i] == columnName) {
        return true;
      }
    }
    return false;
  ...
  getStatusOverview(companyId: any): void {
    ...
    this.vehicleService.getOverviewSpecial(companyId).subscribe({
      next: result => {
        console.log(result);

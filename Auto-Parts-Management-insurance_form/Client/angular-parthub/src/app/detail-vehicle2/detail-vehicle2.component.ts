...
@Component({
  selector: 'app-detail-vehicle2',
  templateUrl: './detail-vehicle2.component.html',
  styleUrls: ['./detail-vehicle2.component.css']
})
export class DetailVehicle2Component implements OnInit {
  ...
  //   },
  //   {
  //     label: 'PARTS', tooltip: 'Vehicle Parts', icon: 'fa-solid fa-cart-shopping', notification: 3,
  ...
  //   },
  //   {
  //     label: 'SUPPS', tooltip: 'Supplements', icon: 'fa-solid fa-list', notification: 19,
  //     selectedTab: 3, sectionId: 3, sectionTooltip: 'Supplements'
  //   },
  //   {
  ...
  bottomMenuSetting = [
    ...
    // {
    //   label: 'SHOP',
    //   tooltip: 'Shop Overview',
    //   icon: 'fa-solid fa-car',
    //   notification: 0,  // Default to 0
    //   selectedTab: 4,
    //   sectionId: 4,
    //   sectionTooltip: 'Shop Overview',
    //   badgeColor: 'bg-danger',
    //   menuTextColor: 'text-white',
    //   menuTextColorActive: 'text-warning'
    // }
    ...
    this.userService.getUserById(userId).subscribe({
      next: result => {
        ...
        if (this.user.companyId != 0 && this.selectedVehicle.companyId == this.user.companyId) {
          ...
          this.getCompanyVendor(this.user.companyId);
          ...
          this.getCompanyExpenseType(this.user.companyId);
          this.getCompanyExpense(this.user.companyId, 0);
  ...
  getCompanyExpense(companyId: any, pageNumber: any): void {
    ...
    this.expenseService.getAllCompanyExpenseWithPage(data).subscribe({
      next: result => {
        ...
        this.setNotification(4, this.expenses.length);
    ...
    this.vehicleService.searchByYearMakeModelColor(data).subscribe({
      next: (res) => {
        ...
        if (type === 7) {
          ...
          //  this.setNotification(4, this.vehiclesOriginal.length);
  ...
  getDetailByUuid(uuid: any): void {
    ...
    this.location.go("/vehicle2/" + uuid);
    ...
    this.vehicleService.getByUuid(uuid).subscribe({
      ...
      next: res => {
        ...
        // this.setNotification(3, this.expenses.length);
        ...
        if (this.selectedVehicle.showInSearchImageId > 0) {
        ...
        if (this.selectedVehicle.id > 0) {
          ...
          this.getVehiclePayments(this.selectedVehicle.id);
    ...
    this.paymentService.getAllVehiclePayments(vehicleId).subscribe({
      next: result => {
        if (result) {
          ...
          this.setNotification(3, this.payments.length);
  ...
  deleteImageExpense(expenseId: any, imageId: any) {
    ...
    this.expenseService.deleteImageWihtUserId(imageId, expenseId, this.currentUser.id).subscribe({
      next: (result) => {
        console.log(result);
        for (let i = 0; i < this.selectedExpense.imageModels.length; i++) {
          if (this.selectedExpense.imageModels[i].id == imageId) {
            this.selectedExpense.imageModels.splice(i, 1);
          }
        }
      }
    });
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      for (let i = 0; i < filesAmount; i++) {
        ...
        reader.onload = (e: any) => {
          ...
          this.uploadImageWithFileExpenses(this.selectedExpense.id, imageModel);
  ...
  private uploadImageWithFileExpenses(expenseId: any, imageModel: ImageModel) {
    ...
    this.expenseService.uploadImageWithFileWithUserId(formData, expenseId, this.user.id).subscribe({
      next: (result) => {
        console.log(result);
        this.selectedExpense!.imageModels!.unshift(result);
      }
    });
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      for (let i = 0; i < filesAmount; i++) {
        ...
        reader.onload = (e: any) => {
          ...
          this.uploadImageWithFilePayments(this.selectedPayment.id, imageModel);
  ...
  private uploadImageWithFilePayments(paymentId: any, imageModel: ImageModel) {
    ...
    this.paymentService.uploadImageWithFileWithUserId(formData, paymentId, this.user.id).subscribe({
      next: (result) => {
        console.log(result);
        this.selectedPayment.imageModels.unshift(result);
      }
    });
  ...
  // setImageForSearchPayments(autopartId: any, imageId: any) {


  //   console.log("setImageForSearchPayments");

  //   this.paymentService.setImageForSearch(imageId, autopartId).subscribe({
  //     next: (result) => {
  //       console.log(result);
  //       for (let i = 0; i < this.payments.imageModels.length; i++) {
  //         if (this.selectedAutopart.imageModels[i].id == imageId) {
  //           this.selectedAutopart.showInSearchImageId = this.selectedAutopart.imageModels[i].id;
  //           //this.selectedImage = this.selectedAutopart.showInSearchImageId;
  //         }
  ...
  deleteImagePayment(paymentId: any, imageId: any) {
    ...
    this.paymentService.deleteImageWihtUserId(imageId, paymentId, this.user.id).subscribe({
      next: (result) => {
        console.log(result);
        for (let i = 0; i < this.selectedPayment.imageModels.length; i++) {
          if (this.selectedPayment.imageModels[i].id == imageId) {
            this.selectedPayment.imageModels.splice(i, 1);
          }
        }
      }
    });

import { AfterViewInit, ChangeDetectorRef, Component, ElementRef, OnInit, Renderer2, HostListener, ɵbypassSanitizationTrustStyle } from '@angular/core';
...
@Component({
  selector: 'app-shop-management-shop',
  templateUrl: './shop-management-shop.component.html',
  styleUrls: ['./shop-management-shop.component.css']
})
export class ShopManagementShopComponent implements OnInit, AfterViewInit {
  ...
  searchCompanies(searchMode: any, name: any, pageNumber: any): void {
    ...
    if (name == "") {
      ...
      this.companyService.getCompaniesWithPage(data).subscribe({
        next: result => {
          console.log(result);
          this.companies = result;
          if (this.companies && this.companies.length > 0) {
            this.searchCount = this.companies[0].searchCount;
          }
      ...
      this.companyService.searchCompaniesWithPage(data).subscribe({
        next: result => {
  ...
  onSearchChangeUser($event: any): void {
    console.log($event.target.value);

    if ($event.target.value.length > 0) {
      this.usersAdmin = new Array();
      this.pagesArray = new Array();

      this.userService.searchUsersByLastName(this.user.companyId, $event.target.value).subscribe({
        next: result => {
          if (result) {
            console.log(result);
            ...
            // if (this.customers.length > 0) 
            {
              ...
              this.messageCount = this.usersAdmin.length + " found";
            }
          }
        }
      })
  ...
  saveUser(user: any): void {
    ...
    if (user.id == 0) {
      ...
      if (window.confirm("Are you sure to update this user ?")) {
        this.userService.updateUserActivate(user.id, user).subscribe({
          next: result => {
            if (result.message != null) {
              this.messageUser = result.message;
  ...
  onChangeCompany($event: any, company: Company): void {

    company.status = $event.target.value;

    this.companyService.createCompany(this.user.id, company).subscribe({
      next: result => {
        console.log(result);
        this.company = result;
      ...
      next: result => {
        ...
        for (let i = 0; i < this.currentUserUser.roles.length; i++) {
          if (this.currentUserUser.roles[i].name == 'ROLE_ADMIN') {
            ...
            this.currentUserUser.allowUpdateUser = true; //for deletion
            ...
            this.getShopNameOverview();
            this.getMarketUserTop100Overview();

            this.getCompaniesWithPage(0, this.pageSize);
  ...
  reportsLastName: GroupBy[] = new Array();

  getShopNameOverview(): void {

    this.companyService.getNameOverview().subscribe({
      next: result => {
        console.log(result);
        this.reportsLastName = result;
  ...
  getCompaniesWithPage(pageNumber: any, pageSize: any) {
    console.log("getCompaniesWithPage");
    this.currantPageNumber = pageNumber;
    ...
    this.companyService.getCompaniesWithPage(data).subscribe({
      next: result => {
        ...
        // for (let company of this.companies) {
        //   this.searchCount = company.searchCount;
        // }
      }
    })
  }
  ...
  reportsMarketUserTop100: User[] = new Array();

  getMarketUserTop100Overview(): void {

    this.autopartService.getMarketUserTop100Overview().subscribe({
      next: result => {
        console.log(result);
        this.reportsMarketUserTop100 = result;
      }
    })
  ...
  getDetailAutopart(autopart: any): void {
  ...
  setCompanyDetail(company: Company, index: any): void {
    this.cindex = index;
  ...
  getCompanyDetails(companyId: any): void {
    ...
    this.getSettings(this.company.id);
    this.getOthers(this.company.id);
        ...
        if (result) {
          ...
          this.userService.getAllCompanyUsers(companyId).subscribe({
            next: result => {
              ...
              if (this.usersCompany.length > 0) {
                this.userIdSnapshot = this.usersCompany[0].id;
              }
            }
          })
        }
  ...
  userIdSnapshot: any;
  usersCompany: User[] = new Array();

  filterUsers(activated: boolean, users: User[]): any {

    return users.filter(u => u.activated == activated).length;
  ...
  public lineChartOptionsVehicleStatus: ChartOptions<'bar'> = {
    ...
    onClick: (
      ...
      if (elements[0]) {
        ...
        for (let status of this.setting.statuss) {
  ...
  searchVehicle(type: number, companyId: any,): void {
  ...
  getCurrentWeekYear(): void {
    ...
    var week: number = Math.ceil(days / 7);
  ...
  setWeekVehicle($event: any): void {
    ...
    this.getVehicleCountOverview(this.company.id, this.from, this.to, this.year, week, this.rangePartMarket);
    this.getCustomerCountOverview(this.company.id, this.from, this.to, this.year, week, this.rangePartMarket);

  }

  totalCountCustomer: any = 0;

  private getOthers(companyId: any) {
    let now = new Date();
    ...
    this.getVehicleCountOverview(companyId, this.from, this.to, this.year, this.week, this.rangePartMarket);
    ...
    this.getCustomerCountOverview(companyId, this.from, this.to, this.year, this.week, this.rangePartMarket);
  ...
  initialLoad() {
    ...
    this.getAutopartCountOverview(0, this.from, this.to, this.year, this.week, this.rangePartMarket);
  ...
  getImageCountOverview(companyId: any, from: Date, to: Date, year: any, week: any, range: any): void {
    ...
    this.autopartService.getImageCountOverview(dateCarrier).subscribe({
      next: result => {
        // console.log(result);
        var reportCarrier: ReportCarrier = result;
        var lables: any[] = new Array();
  ...
  getUserCountOverview(companyId: any, from: Date, to: Date, year: any, week: any, range: any): void {
    ...
    this.userService.getUserCountOverview(companyId, dateCarrier).subscribe({
      next: result => {
        // console.log(result);
        var reportCarrier: ReportCarrier = result;
        var lables: any[] = new Array();
  ...
  getAutopartCountOverview(companyId: any, from: Date, to: Date, year: any, week: any, range: any): void {
    ...
    this.autopartService.getAutopartCountOverview(dateCarrier).subscribe({
      next: result => {
        // console.log(result);
        var reportCarrier: ReportCarrier = result;
        var lables: any[] = new Array();

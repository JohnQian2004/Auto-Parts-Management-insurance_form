...
export class DisclaimerComponent implements OnInit, AfterViewInit {
  ...
  createDisclaimer(disclaimer: Disclaimer): void {
    ...
    if (disclaimer.name == null || disclaimer.name == '' ||
      disclaimer.text == null || disclaimer.text == ''
    ) {
      this.messageDisclaimer = "Disclaimer name and text are required";
      return;
    }

    if (disclaimer.name != null && disclaimer.name != '' && disclaimer.name.length > 255) {

      this.messageDisclaimer = "disclaimer Name is too long";
      return;
    }

    if (disclaimer.comments != null && disclaimer.comments != '' && disclaimer.comments.length > 255) {

      this.messageDisclaimer = "Disclaimer Comments are too long";
      return;
    }


    if (disclaimer.text != null && disclaimer.text != '' && disclaimer.text.length > 7000) {

      this.messageDisclaimer = "disclaimer text is too long";
      return;
    }

    if (disclaimer.name != null && disclaimer.name != '' && disclaimer.name.length < 2) {

      this.messageDisclaimer = "disclaimer Name is too short";
      return;
    }

    if (disclaimer.comments != null && disclaimer.comments != '' && disclaimer.comments.length < 2) {

      this.messageDisclaimer = "Disclaimer Comments are too short";
      return;
    }


    if (disclaimer.text != null && disclaimer.text != '' && disclaimer.text.length < 2) {

      this.messageDisclaimer = "disclaimer text is too short";
      return;
    }
    ...
    if (disclaimer.id == null || disclaimer.id == 0)
      newEntry = true;
    ...
    this.disclaimerService.createDisclaimer(this.currentUser.id, disclaimer).subscribe({
      next: result => {
        ...
        if (newEntry == true)
          this.messageDisclaimer = "Successfully Created";
        else
  ...
  deleteDisclaimer(disclaimerId: any): void {

    this.disclaimerService.deleteDisclaimer(disclaimerId).subscribe({
      next: result => {

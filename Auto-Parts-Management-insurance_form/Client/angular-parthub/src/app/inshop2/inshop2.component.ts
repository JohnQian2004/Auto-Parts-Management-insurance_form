...
@Component({
  selector: 'app-inshop2',
  templateUrl: './inshop2.component.html',
  styleUrls: ['./inshop2.component.css']
})
export class Inshop2Component implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.eventBusSub = this.eventBusService.on('display', (data: any) => {
      ...
      this.noteService.getAllCompanyNote(this.company.id).subscribe({
        next: result => {
          this.notes = result;
              ...
              this.jobService.createJob(this.user.id, this.job).subscribe({
                next: result => {
                  if (result) {
                    ...
                    // this.noteService.getAllCompanyNote(this.user.companyId);
                    this.getDetail(this.vehicle, 0);
                    this.vehicles = this.vehiclesOriginal.filter(vehicle =>
                      vehicle.year == this.vehicle.year &&
                      vehicle.make == this.vehicle.make &&
                      vehicle.model == this.vehicle.model);
                    // this.printPageJob('print-content-job', 'jobs', result)
                  }
                }
              })
    ...
    this.eventBusSub = this.eventBusService.on('print', (data: any) => {
      console.log(" app event " + data);
      this.noteService.getAllCompanyNote(this.company.id).subscribe({
        next: result => {
          this.notes = result;

          this.setJobNotifiedAndPrint(data);
    ...
    this.anylyticalService.trackEvent('loaded', 'loaded', 'inshop');
  ...
  onSearchInput(searchInput: any): void {
    // console.log($event.target.value);
    if (searchInput != '') {
      if (this.vehiclesOriginal.length > 0)
        this.vehicles = this.vehiclesOriginal.filter(vehicle => vehicle.serachString.toLowerCase().includes(searchInput.toLocaleLowerCase()));
    }
  }
  ...
  addToAiImageArray(imageIdIn: any): void {
    var hasIt = false;
    for (let i = 0; i < this.aiImages.length; i++) {
      if (this.aiImages[i] == imageIdIn) {
        hasIt = true;
        this.aiImages.splice(i, 1);
      }
    }
    if (hasIt == false) {
      this.aiImages.push(imageIdIn);
    }
    console.log(this.aiImages);
  }

  autopartAi: AutoPart = new AutoPart();
  claimAi: Claim = new Claim();
  jobAi: Job = new Job();

  indexAi: any = 0;

  setDetailAutopartAi(autopart: AutoPart, index: any): void {
    this.autopartAi = autopart;
    this.indexAi = index;
  }

  setDetailJobAi(job: Job, index: any): void {
    this.jobAi = job;
    this.indexAi = index;
  }

  setDetailClaimAi(claim: Claim, index: any): void {
    this.claimAi = claim;
    this.indexAi = index;
  }

  addClaimFromAi(claim: Claim): void {
  ...
  addJobFromAi(job: Job): void {

    job.vehicleId = this.vehicle.id;
    job.userId = this.user.id;

    this.jobService.createJob(this.user.id, job).subscribe({
      next: result => {
        this.jobAi = result;
        job.id = this.jobAi.id;
        this.getVehicleJobs2(this.vehicle.id);
      }
    })
  }

  addAutopartFromAi(autopart: AutoPart): void {

    autopart.vehicleId = this.vehicle.id;
    autopart.userId = this.user.id;
    autopart.year = this.vehicle.year;
    autopart.make = this.vehicle.make;
    autopart.model = this.vehicle.model;
    autopart.salePrice = autopart.price;


    this.autopartService.create(autopart).subscribe({
      next: result => {
        this.autopartAi = result;
        autopart.id = this.autopartAi.id;
        this.getAutopartForVehicle(this.vehicle.id, false);
      }
    })
  }
  ...
  disenabledGetAiEstimate: boolean = false;

  getAiEstimate(): void {
    var data = {
      vehicleId: this.vehicle.id,
      images: this.aiImages,
      // year: this.vehicle.year,
      // make: this.vehicle.make,
      // model: this.vehicle.model,
      workRequest: this.vehicle.year + " " + this.vehicle.make + " " + this.vehicle.model + ": " + this.vehicle.workRequest
    }
    this.disenabledGetAiEstimate = true;

    this.claimService.getAiEstimate(data).subscribe({
      next: result => {
        console.log(result);
        this.estimateResponse = result;
        this.disenabledGetAiEstimate = false;
      }
    })
  }
  ...
  deleteAutopart(autoPart: AutoPart, index: any): void {
    ...
    var receiptNumber = 0;
    for (let receipt of this.receipts) {
      if (receipt.autopartId == autoPart.id) {
        hasReceipt = true;
    ...
    if (hasReceipt == true) {
      message = 'Remove receipt [' + receiptNumber + '] as well ?';
      ...
      this.confirmationService.confirm(message, customTitle, buttonType, (confirmed: boolean) => {
        if (confirmed == undefined) {
          ...
          this.autopartService.deleteWithOptionUserId(autoPart.id, this.user.id).subscribe({
            next: data => {
              console.log(" " + data);
              if (this.autopartsSearch.length > 0) {
                for (let i = 0; i < this.autopartsSearch.length; i++) {
  ...
  sortList(fieldName: any): void {
    ...
    if (fieldName == 'insuranceCompany') {
      this.vehicles = this.vehicles.filter(item => item['insuranceCompany'] !== null);
      if (this.counter % 2 == 1)
    ...
    if (fieldName == 'serviceManager') {

      for (let vehicle of this.vehicles) {
        vehicle.sortStr = "";
        for (let employee of this.employees) {
          if (employee.id == vehicle.serviceManager) {
            vehicle.sortStr = employee.firstName + employee.lastName;
          }
        }
      }
  ...
  getTotalBallance(): number {
    return +((this.getSubtotal() + this.getTax()) - this.getTotalPayments()).toFixed(2);
  }
        ...
        if (employee.commissionBased == false) {
          ...
          if (job.claimId > 0) {
            ...
            if (employee.hourRate > 0) {
              job.hours = 0;
              job.price = +(job.hours * employee.hourRate).toFixed(0);
            } else {
              job.hours = 0;
  ...
  shallDisplayHours(job: Job): boolean {
    for (let employee of this.employees) {
  ...
  onChangeJobHour($event: any, job: Job) {
    ...
    //     if (this.currentEmplyeeId == job.employeeId) {
    //       this.getMyJobs(job.employeeId);
    //     }
  ...
  onChangeStatusOverview($event: any, status: string): void {
  ...
  printPageQRCode(componentId: string, title: any) {
    ...
    WindowPrt?.document.write("<link href=\"../styles.css\" rel=\"stylesheet\">")
    WindowPrt?.document.write('<style type=\"text/css\">body{background-color: white;}</style>');
    WindowPrt?.document.write('<script src="https://cdn.jsdelivr.net/npm/qrcode/build/qrcode.min.js"></script>');
    // WindowPrt?.document.write('<style type=\"text/css\">th{color: white;background: rgb(13, 173, 226);}</style>');
    WindowPrt?.document.write(`<script>
      QRCode.toCanvas(document.getElementById('qrcode'), '${this.url}', function (error) {

...
import { WarrantyService } from '../_services/warranty.service';
...
export class WarrantyComponent implements OnInit, AfterViewInit {
  ...
  createWarranty(warranty: Warranty): void {

    warranty.companyId = this.company.id;

    if (warranty.name == null || warranty.name == '' ||
      warranty.text == null || warranty.text == ''
    ) {
      this.messageWarranty = "Warranty name and text are required";
      return;
    }

    if (warranty.name != null && warranty.name != '' && warranty.name.length > 255) {

      this.messageWarranty = "warranty Name is too long";
      return;
    }

    if (warranty.comments != null && warranty.comments != '' && warranty.comments.length > 255) {

      this.messageWarranty = "Warranty Comments are too long";
      return;
    }


    if (warranty.text != null && warranty.text != '' && warranty.text.length > 7000) {

      this.messageWarranty = "warranty text is too long";
      return;
    }

    if (warranty.name != null && warranty.name != '' && warranty.name.length < 2) {

      this.messageWarranty = "warranty Name is too short";
      return;
    }

    if (warranty.comments != null && warranty.comments != '' && warranty.comments.length < 2) {

      this.messageWarranty = "Warranty Comments are too short";
      return;
    }


    if (warranty.text != null && warranty.text != '' && warranty.text.length < 2) {

      this.messageWarranty = "warranty text is too short";
      return;
    }

    if (warranty.id == 0 && this.warranties.length == 0)
      warranty.isDefault = true;

    if (warranty.id > 0 && this.warranties.length == 1)
      warranty.isDefault = true;

    var newEntry: boolean = false;
    if (warranty.id == null || warranty.id == 0)
      newEntry = true;

    this.warrantyService.createWarranty(this.currentUser.id, warranty).subscribe({
      next: result => {
        this.warranty = result;
        if (newEntry == true)
          this.messageWarranty = "Successfully Created";
        else
          this.messageWarranty = "Successfully Updated";
        this.getAllWarranty(this.company.id);
      }
    })
  }
  ...
  getAllWarranty(companyId: any): void {

    this.warranties = new Array();

    this.warrantyService.getAllCompanyWarranty(companyId).subscribe({
      next: result => {
        //if (result)
        this.warranties = result;
      }
    })
  }
  ...
  getWarrantyDetail(warranty: Warranty, index: any): void {
  ...
  deleteWarranty(warrantyId: any): void {

    this.warrantyService.deleteWarranty(warrantyId).subscribe({
      next: result => {

...
@Component({
  selector: 'app-pre-defined-jobs',
  templateUrl: './pre-defined-jobs.component.html',
  styleUrls: ['./pre-defined-jobs.component.css']
})
export class PreDefinedJobsComponent implements OnInit, AfterViewInit {
  ...
  createService(service: Service): void {
    ...
    if (service.name == null || service.name == '') {
      this.messagePredefinedJobs = "Pre-defined Job Name is required";
      return;
    }

    if (service.name != null && service.name != '' && service.name.length < 2) {

      this.messagePredefinedJobs = "Pre-defied Job Name is too short";
      return;
    }

    if (service.comments != null && service.comments != '' && service.comments.length < 2) {

      this.messagePredefinedJobs = "Pre-defined Job Comments are too short";
      return;
    }

    if (service.name != null && service.name != '' && service.name.length > 255) {

      this.messagePredefinedJobs = "Pre-defied Job Name is too long";
      return;
    }

    if (service.comments != null && service.comments != '' && service.comments.length > 255) {

      this.messagePredefinedJobs = "Pre-defined Job Comments are too long";
      return;
    }
    ...
    this.serviceService.createService(this.currentUser.id, service).subscribe({
      next: result => {
        this.service = result;
        if (newEntry == true)
          this.messagePredefinedJobs = "Successfully Created";
        else

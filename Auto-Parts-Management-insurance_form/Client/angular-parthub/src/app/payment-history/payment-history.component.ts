...
import { DayDataCarrier } from '../models/day.data.carrier';
...
export class PaymentHistoryComponent implements OnInit {
  ...
  public lineChartOptionsVehicle: ChartOptions<'bar'> = {
    ...
    onClick: (
      ...
      if (elements[0]) {
        ...
        var week = this.lineVehicleChartData.labels![elements[0].index];

        this.getThisWeekPaymentHistory(this.year, week);
  ...
  errorMessagePayrollHistory: any = "";

  onSearchJobId($event: any): void {
    this.payrollHistory = new PayrollHistory();
    this.errorMessagePayrollHistory = "";
    console.log("onSearchJobId");
    var jobId = $event.target.value;
    if (jobId != null) {
      this.payrollHistoryService.searchPayrollHistory(jobId, this.user.companyId).subscribe({
        next: result => {
          console.log(result);
          this.payrollHistory = result;
          if (this.payrollHistory.vehicle == null && this.payrollHistory.job == null) {
            this.errorMessagePayrollHistory = "Job # [ " + jobId + " ] Not Found";
          }
        }
      })
    }
  ...
  getDetailJob(jobId: any): void {
    this.payrollHistory = new PayrollHistory();
    this.errorMessagePayrollHistory = "";
    console.log("getDetailJob");
    if (jobId != null) {
      this.payrollHistoryService.searchPayrollHistory(jobId, this.user.companyId).subscribe({
        next: result => {
          console.log(result);
  ...
  getPayrollHistoryDetail(payrollHistory: any, index: any): void {
  ...
  searchJobId(jobId: any): void {
    this.payrollHistory = new PayrollHistory();
    this.errorMessagePayrollHistory = "";
    if (jobId) {
      console.log("searchJobId");
  ...
  onEnterJobPrice(reason: any, payrollHistory: PayrollHistory): void {
    payrollHistory.job.reason = reason;
    this.jobService.createJob(this.currentUser.id, payrollHistory.job).subscribe({
      next: result => {
        payrollHistory.job = result;
  ...
  searchJobIdAndAddToPayroll(jobId: any): void {


    const customTitle = 'Add To Payroll';
    const message = 'Are you sure you want to add Job [' + jobId + '] to payroll ?';
    ...
    this.confirmationService.confirm(message, customTitle, buttonType, (confirmed: boolean) => {
      if (confirmed) {
        ...
        this.errorMessagePayrollHistory = "";
        console.log("searchJobIdAndAddToPayroll");
        if (jobId != null) {
          this.payrollHistoryService.searchPayrollHistory(jobId, this.user.companyId).subscribe({
            next: result => {
              console.log(result);
              ...
              payrollHistory.year = this.year;
              payrollHistory.week = this.week;
              payrollHistory.companyId = this.user.companyId;
              payrollHistory.jobId = jobId;

              this.payrollHistoryService.createPayrollHistory(this.user.id, payrollHistory).subscribe({
                next: result => {
                  payrollHistory = result;
                  this.getThisWeekPayrollHistory(this.year, this.week);
                  this.findAllCurrentEmplyeeJobs();
  ...
  searchJobIdAndRemoveFromPayroll(jobId: any): void {

    const customTitle = 'Remote From Payroll';
    const message = 'Are you sure you want to remote Job [' + jobId + '] from payroll ?';
    const buttonType = "yesNo" //buttonType: 'yesNo' | 'okCancel' | 'okOnly' = 'yesNo';  // Button type
    ...
    this.confirmationService.confirm(message, customTitle, buttonType, (confirmed: boolean) => {
      if (confirmed) {
        var payrollHistory = new PayrollHistory();
        this.errorMessagePayrollHistory = "";
        console.log("searchJobIdAndRemeveFromPayroll");
        if (jobId != null) {
          this.payrollHistoryService.searchPayrollHistory(jobId, this.user.companyId).subscribe({
            next: result => {
              console.log(result);
              ...
              this.payrollHistoryService.deletePayrollHistory(payrollHistory.id).subscribe({
                next: result => {
  ...
  addToPayroll(payrollHistory: any): void {


    const customTitle = 'Add To Payroll';
    const message = 'Are you sure you want to add Job [' + payrollHistory.job.id + '] to payroll ?';
    const buttonType = "yesNo" //buttonType: 'yesNo' | 'okCancel' | 'okOnly' = 'yesNo';  // Button type
    ...
    this.confirmationService.confirm(message, customTitle, buttonType, (confirmed: boolean) => {
      if (confirmed) {
        ...
        payrollHistory.week = this.week;
        payrollHistory.companyId = this.user.companyId;
        payrollHistory.jobId = payrollHistory.job.id;
  ...
  printPageJob(componentId: string, title: any, job: Job, vehicle: any) {
    this.job = job;
  ...
  printPagePayrollHistory(componentId: string, title: any) {
  ...
  removeFromPayroll(payrollHistory: any): void {

    const customTitle = 'Remote From Payroll';
    const message = 'Are you sure you want to remote Job [' + payrollHistory.job.id + '] from payroll ?';
    const buttonType = "yesNo" //buttonType: 'yesNo' | 'okCancel' | 'okOnly' = 'yesNo';  // Button type
  ...
  getThisWeekPayrollHistory(year: any, week: any): void {
    ...
    this.payrollHistoryService.getAllCompanyPayrollHistoies(this.user.companyId, year, week).subscribe({
      next: result => {
        console.log(result);
        ...
        for (let employee of this.employeeJobCarrier) {
          ...
          if (employee.jobs) {
            ...
            for (let j = 0; j < employee.jobs.length; j++) {
              ...
              if (this.isInPayrollHistory(employee.jobs[j].id)) {
                this.totalPaidJob++;
              } else {
                if (employee.jobs[j].paidWeek == 0) {
                  this.totalUnPaidJob++;
                }
              }
  ...
  getPaidUnPaidJobTotalCounts(paidOrUnPaid: boolean): any {
    ...
    if (paidOrUnPaid == true) {
      return this.totalPaidJob;
    } else {
      return this.totalUnPaidJob;
    }
  }
  ...
  totalPaidJob: any = 0;
  totalUnPaidJob: any = 0;

  filterJobs2(employee: Employee, paidOrNot: boolean): Vehicle[] {
    var vehicles: Vehicle[] = new Array();
    var jobs: any[] = new Array();
    ...
    jobs.forEach(job => {
      ...
      if (paidOrNot == false) {
        if (this.isInPayrollHistory(job.id) == false && job.paidWeek == 0) {
          jobMap[job.vehicleId].push(job);
          //this.totalUnPaidJob++;
        }
      } else {
        if (this.isInPayrollHistory(job.id) == true && job.paidWeek > 0) {
          jobMap[job.vehicleId].push(job);
          //this.totalPaidJob++;
        }
      }
    ...
    this.vehicles.forEach(vehicle => {
      ...
      if (jobMap[vehicle.id]) {
        ...
        if (paidOrNot == false)
          ...
          vehicle.jobs3 = jobMap[vehicle.id];
  ...
  getTotalPay(employeeId: any): any {

    var total = 0;
    if (this.payrollHistories && this.payrollHistories.length > 0) {
      for (let payrollHistory of this.payrollHistories) {
        if (payrollHistory.job.employeeId == employeeId) {
  ...
  getTotalJobCounts(employeeId: any): any {
    ...
    this.settingService.getSetting(companyId).subscribe({
      next: result => {
        if (result) {
          ...
          if (this.currentUserUser.allowAddPayment == true) {
            this.getOthers();
          }
  ...
  private getOthers() {
    ...
    //this.getThisWeekPayrollHistory(this.year, this.week);
    this.getThisWeekPaymentHistory(this.year, this.week);
    this.getVehiclePaymentOverview(this.year, this.week);
    this.searchVehicle(7);
    this.findAllCurrentEmplyeeJobs();

  }

  dayDataCarriers: DayDataCarrier[] = new Array()

  getThisWeekPaymentHistory(year: any, week: any): void {
    ...
    this.dayDataCarriers = new Array();

    this.paymentService.getWeeklyPayment(this.currentUserUser.companyId, data).subscribe({
      next: result => {
        console.log(result);
        this.dayDataCarriers = result;
        this.calendarKicker();
      }
    })
  }

  getPaymentCounts(): any {
    var count = 0;
    for (let dayDataCarrier of this.dayDataCarriers) {
      count += dayDataCarrier.data.length;
    }
    return count;
  }

  getWeeklyPaymentTotals(): any {
    var total = 0;
    for (let dayDataCarrier of this.dayDataCarriers) {
      for (let payment of dayDataCarrier.data) {
  ...
  getDailyPaymentTotals(dayDataCarrier: DayDataCarrier): any {
    var total = 0;
    for (let payment of dayDataCarrier.data) {
      total += payment.amount;
    }
  ...
  setWeek($event: any) {
    ...
    //this.payrollHistory = new PayrollHistory();
    //this.getThisWeekPayrollHistory(year, this.weekSet);
    //getThisWeekPaymentHistory
    this.week = this.weekSet;
    this.getVehiclePaymentOverview(this.year, this.week);
    this.getThisWeekPaymentHistory(year, this.weekSet);
  ...
  getVehiclePaymentOverview(year: any, week: any): void {
    ...
    this.paymentService.getVehiclePaymentOverview(this.currentUserUser.companyId, dateCarrier).subscribe({
      next: result => {
        console.log(result);
        var reportCarrier: ReportCarrier = result;
        var lables: any[] = new Array();
  ...
  onEnterPayrollHistory(reason: any, job: Job): void {
    job.reason = reason;
    this.jobService.createJob(this.currentUser.id, job).subscribe({
      next: result => {
        job = result;

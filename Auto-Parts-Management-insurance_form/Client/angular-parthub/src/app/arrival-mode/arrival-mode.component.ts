...
@Component({
  selector: 'app-arrival-mode',
  templateUrl: './arrival-mode.component.html',
  styleUrls: ['./arrival-mode.component.css']
})
export class ArrivalModeComponent implements OnInit, AfterViewInit {
  ...
  createInTakeWay(inTakeWay: InTakeWay): void {
    ...
    if (inTakeWay.name != null && inTakeWay.name != '' && inTakeWay.name.length < 2) {

      this.messageInTakeWay = "Arrival Mode Name is too short";
      return;
    }
    ...
    if (inTakeWay.comments != null && inTakeWay.comments != '' && inTakeWay.comments.length < 2) {

      this.messageInTakeWay = "Arrival Mode Comments are too short";
      return;
    }

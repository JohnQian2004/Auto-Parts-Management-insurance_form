...
export class DocTypeComponent implements OnInit, AfterViewInit {
  ...
  getAllDocType(companyId: any): void {

    this.docTypes = new Array();

    this.docTypeService.getAllCompanyDocType(companyId).subscribe({
      next: result => {
  ...
  createPaymentStatus(paymentStatus: PaymentStatus): void {
    ...
    if (paymentStatus.name == null || paymentStatus.name == '') {
      this.messagePaymentStatus = "Payment Status name is required";
      return;
    }
  ...
  createDocType(docType: DocType): void {

    docType.companyId = this.company.id;

    if (docType.name == null || docType.name == '') {
      this.messageDocType = "Document Name is required";
      return;
    }

    if (docType.name != null && docType.name != '' && docType.name.length < 2) {

      this.messageDocType = "Document Name is too short";
      return;
    }

    if (docType.name != null && docType.name != '' && docType.name.length > 255) {

      this.messageDocType = "Document Name is too long";
      return;
    }

    if (docType.comments != null && docType.comments != '' && docType.comments.length < 2) {

      this.messageDocType = "Pament Method Comments are too short";
      return;
    }


    if (docType.comments != null && docType.comments != '' && docType.comments.length > 255) {

      this.messageDocType = "Pament Method Comments are too long";
      return;
    }

    var newEntry: boolean = false;
    if (docType.id == null || docType.id == 0)
      newEntry = true;

    this.docTypeService.createDocType(this.currentUser.id, docType).subscribe({
      next: result => {
        this.docType = result;
        if (newEntry == true)
          this.messageDocType = "Successfully Created";
        else
          this.messageDocType = "Successfully Updated";
        this.getAllDocType(this.company.id);
      }
    })
  }
  ...
  createPaymentMethod(paymentMethod: PaymentMethod): void {
    ...
    if (paymentMethod.name == null || paymentMethod.name == '') {
      this.messagePaymentMethod = "Payment Method Name is required";
      return;
    }

    if (paymentMethod.name != null && paymentMethod.name != '' && paymentMethod.name.length < 2) {

      this.messagePaymentMethod = "Payment Method Name is too short";
      return;
    }

    if (paymentMethod.name != null && paymentMethod.name != '' && paymentMethod.name.length > 255) {

      this.messagePaymentMethod = "Payment Method Name is too long";
      return;
    }

    if (paymentMethod.comments != null && paymentMethod.comments != '' && paymentMethod.comments.length < 2) {

      this.messagePaymentMethod = "Pament Method Comments are too short";
      return;
    }


    if (paymentMethod.comments != null && paymentMethod.comments != '' && paymentMethod.comments.length > 255) {

      this.messagePaymentMethod = "Pament Method Comments are too long";
      return;
    }

    var newEntry: boolean = false;
    if (paymentMethod.id == null || paymentMethod.id == 0)
      newEntry = true;
    ...
    this.paymentMethodService.createPaymentmethod(this.currentUser.id, paymentMethod).subscribe({
      next: result => {
        ...
        if (newEntry == true)
          this.messagePaymentMethod = "Successfully Created";
        else
  ...
  getDocTypeDetail(docType: DocType, index: any): void {
  ...
  deleteDocType(): void {
    ...
    this.docTypeService.deleteDocType(this.docType.id).subscribe({
      next: result => {

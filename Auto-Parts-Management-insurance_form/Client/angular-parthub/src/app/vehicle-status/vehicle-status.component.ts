...
export class VehicleStatusComponent implements OnInit, AfterViewInit {
  ...
  createApprovalStatus(approvalStatus: ApprovalStatus): void {
    ...
    if (approvalStatus.name == null || approvalStatus.name == '') {
      ...
      this.messageApprovalStatus = "Status Name is required";
      return;
    }

    if (approvalStatus.name != null && approvalStatus.name != '' && approvalStatus.name.length > 200) {
      this.messageApprovalStatus = "Status Name is too long";
      return;
    }

    if (approvalStatus.name != null && approvalStatus.name != '' && approvalStatus.name.length < 2) {
      this.messageApprovalStatus = "Status Name is too Short";
      return;
    }

    if (approvalStatus.comments != null && approvalStatus.comments != '' && approvalStatus.comments.length > 200) {
      this.messageApprovalStatus = "Status comments is too long";
      return;
    }

    if (approvalStatus.comments != null && approvalStatus.comments != '' && approvalStatus.comments.length < 2) {
      this.messageApprovalStatus = "Status comments is too Short";
      return;
    }

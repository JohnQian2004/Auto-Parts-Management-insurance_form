...
export class ApprovalStatusComponent implements OnInit, AfterViewInit {
  ...
  createInTakeWay(inTakeWay: InTakeWay): void {
    ...
    if (inTakeWay.name == null || inTakeWay.name == '') {

      this.messageInTakeWay = "Arrival Mode Name is required";
      return;
    }

    if (inTakeWay.name != null && inTakeWay.name != '' && inTakeWay.name.length > 255) {

      this.messageInTakeWay = "Arrival Mode Name is too long";
      return;
    }

    if (inTakeWay.comments != null && inTakeWay.comments != '' && inTakeWay.comments.length > 255) {

      this.messageInTakeWay = "Arrival Mode Comments are too long";
      return;
    }


    var newEntry: boolean = false;
    if (inTakeWay.id == null || inTakeWay.id == 0)
      newEntry = true;
    ...
    this.inTakeWayService.createInTakeWay(this.currentUser.id, inTakeWay).subscribe({
      next: result => {
        ...
        if (newEntry == true)
          this.messageInTakeWay = "Successfully Created";
        else
  ...
  createApprovalStatus(approvalStatus: ApprovalStatus): void {
    ...
    if (approvalStatus.name == null || approvalStatus.name == '') {

      this.messageApprovalStatus = "Approval Status Name is required";
      return;
    }

    if (approvalStatus.name != null && approvalStatus.name != '' && approvalStatus.name.length > 255) {

      this.messageApprovalStatus = "Approval Status Name is too long";
      return;
    }

    if (approvalStatus.comments != null && approvalStatus.comments != '' && approvalStatus.comments.length > 255) {

      this.messageApprovalStatus = "Approval Status Comments are too long";
      return;
    }

    if (approvalStatus.name != null && approvalStatus.name != '' && approvalStatus.name.length < 2) {

      this.messageApprovalStatus = "Approval Status Name is too short";
      return;
    }

    if (approvalStatus.comments != null && approvalStatus.comments != '' && approvalStatus.comments.length < 2) {

      this.messageApprovalStatus = "Approval Status Comments are too short";
      return;
    }

    var newEntry: boolean = false;
    if (approvalStatus.id == null || approvalStatus.id == 0)
      newEntry = true;
    ...
    this.approvalStatusService.createApprovalStatus(this.currentUser.id, approvalStatus).subscribe({
      next: result => {
        ...
        if (newEntry == false)
          ...
          this.messageApprovalStatus = "Successfully Created"

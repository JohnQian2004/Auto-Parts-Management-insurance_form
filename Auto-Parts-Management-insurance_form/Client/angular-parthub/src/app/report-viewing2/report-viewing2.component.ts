...
@Component({
  selector: 'app-report-viewing2',
  templateUrl: './report-viewing2.component.html',
  styleUrls: ['./report-viewing2.component.css']
})
export class ReportViewing2Component implements OnInit, OnDestroy {
  ...
  // Vehicle detail editor properties
  showVehicleEditor: boolean = false;
  selectedVehicleForEditor: Vehicle | null = null;

  // Vehicle editor event handlers
  onVehicleEditorClosed(): void {
    console.log('Vehicle editor closed');
    this.showVehicleEditor = false;
    this.selectedVehicleForEditor = null;
  }

  onVehicleUpdated(event: { vehicleId: number | string }): void {
    console.log('Vehicle updated:', event.vehicleId);
    // Refresh the vehicle list or update the specific vehicle
    this.loadCurrentShopSummaryReport();
  }

  onVehicleArchived(event: { vehicleId: number | string }): void {
    console.log('Vehicle archived:', event.vehicleId);
    // Refresh the vehicle list
    this.loadCurrentShopSummaryReport();
  }

  onVehicleUnarchived(event: { vehicleId: number | string }): void {
    console.log('Vehicle unarchived:', event.vehicleId);
    // Refresh the vehicle list
    this.loadCurrentShopSummaryReport();
  }
  ...
  getDetail(vehicle: Vehicle, index: any): void {
    console.log('Opening vehicle detail editor for vehicle:', vehicle);

    // Set the selected vehicle for the editor
    this.selectedVehicleForEditor = vehicle;
    this.showVehicleEditor = true;
    this.vehicle = vehicle;

  }

  getDetail11(vehicle: Vehicle, index: any): void {
    console.log('🚗 NEW APPROACH: Pre-fetching payments for vehicle:', vehicle);

    // Clone the vehicle to avoid modifying the original
    this.selectedVehicleForEditor = { ...vehicle };
    this.showVehicleEditor = false; // Don't show until payments are loaded

    // Pre-fetch payments and attach to vehicle object
    console.log('🔄 Fetching payments for vehicle ID:', vehicle.id);
    this.paymentService.getAllVehiclePayments(vehicle.id).subscribe({
      next: (payments) => {
        console.log('💰 Payments fetched:', payments);
        // Attach payments directly to the vehicle object
        this.selectedVehicleForEditor!.payments = payments || [];
        console.log('✅ Vehicle with payments ready:', this.selectedVehicleForEditor);
      ...
      error: (error) => {
        ...
        // Show editor anyway with empty payments
        this.selectedVehicleForEditor!.payments = [];
        this.showVehicleEditor = true;
      }
    });

    // Keep existing properties for backward compatibility (if needed)
    this.vehicle = vehicle;
      ...
      next: (result) => {
        ...
        this.paymentService.getPayment(paymentId).subscribe({
          next: result => {
            this.payment = result;
            for (let i = 0; i < this.payments.length; i++) {
              if (this.payments[i].id == paymentId) {

...
export class InventoryComponent implements OnInit {
  ...
  refresh(): void {
    ...
    const scrollContainer = this.el.nativeElement.querySelector('.scroller');

    // Use Renderer2 to add the 'wheel' event listener
    this.renderer.listen(scrollContainer, 'wheel', (evt: WheelEvent) => {
      evt.preventDefault();
  ...
  setSearchBar(showSearchBar: boolean): void {
  ...
  setArchivedFlag(archivedFlag: boolean): void {
  ...
  getAllAutopartsWithPage(pageNumber: any, pageSize: any, status: any, archived: boolean): void {
    ...
    //   if (pageNumber >= this.pagesArrayArchived.length) {
    //     pageNumber = this.pagesArrayArchived.length - 1;
    //   }
    // }
    // else {
    //   if (pageNumber >= this.pagesArrayInventory.length) {
    //     pageNumber = this.pagesArrayInventory.length - 1;
    //   }
    // }
    ...
    if (archived == true) {
      ...
      pageNumber = Math.max(this.currantPageNumberArchived - 1, 0);
    }
    else {
      this.currantPageNumberInventory = pageNumber;
      pageNumber = Math.max(this.currantPageNumberInventory - 1, 0);
    }
  ...
  deleteAutopartWithStatusInventory(autopart: AutoPart, status: any): void {
    this.autopartService.delete(autopart.id).subscribe({
      next: result => {
        //this.getCompanyAutopart(this.user.companyId, status);
        this.getAllAutopartsWithPage(this.currantPageNumberInventory, this.pageSizeInventory, status, false);
      }
    })
  }


  deleteAutopartWithStatusArchived(autopart: AutoPart, status: any): void {
    this.autopartService.delete(autopart.id).subscribe({
      next: result => {
        //this.getCompanyAutopart(this.user.companyId, status);
        this.getAllAutopartsWithPage(this.currantPageNumberArchived, this.pageSizeArchived, status, true);
      }
    })
  }
  ...
  archiveAutopart(autopart: AutoPart, archive: boolean, reason: any): void {
    ...
    this.autopartService.update(autopart.id, autopart).subscribe({
      next: result => {
        //this.getCompanyAutopart(this.user.companyId, 3);
        if (archive == false) {
          this.getAllAutopartsWithPage(this.currantPageNumberArchived, this.pageSizeArchived, 0, false)
        } else {
  ...
  filterAutoparts(status: any): AutoPart[] {
    ...
    if (this.autoparts != null && this.autoparts.length > 0)
      return this.autoparts.filter(autopart => autopart.status === status);
    else {
      this.autoparts = new Array();
  ...
  saveAutopart(): void {
    ...
    if (this.autopart.title == null || this.autopart.title == '' ||
      this.autopart.description == null || this.autopart.description == '' ||
      this.autopart.price == null || this.autopart.price == 0 ||
      this.autopart.grade == null || this.autopart.grade == '' ||
      this.autopart.vendorId == null || this.autopart.vendorId == 0 ||
      this.autopart.paymentMethodId == null || this.autopart.paymentMethodId == 0) {

      this.messagePart = "Please fill the form. Name, description, price, grade, vendor and payment method are required";
      return;
    }

    if (this.autopart.title == null || this.autopart.title == '') {
      this.messagePart = "Parts Name is required";
      return;
    }

    if (this.autopart.title != null && this.autopart.title != '' && this.autopart.title.length < 2) {
      this.messagePart = "Parts Name is too short";
      return;
    }

    if (this.autopart.description == null || this.autopart.description == '') {
      this.messagePart = "Part Description is required";
      return;
    }

    if (this.autopart.description != null && this.autopart.description != '' && this.autopart.description.length < 2) {
      this.messagePart = "Parts Description is too short";
      return;
    }


    // if (this.autopart.shipping == null || this.autopart.shipping == '') {
    //   this.messagePart = "Part Shipping is required";
    //   return;
    // }

    // if (this.autopart.warranty == null || this.autopart.warranty == '') {
    //   this.messagePart = "Part Warranty is required";
    //   return;
    // }

    if (this.autopart.grade == null || this.autopart.grade == '') {
      this.messagePart = "Part Grade is required";
      return;
    }

    if (this.autopart.price == null || (this.autopart.price != null && this.autopart.price == 0)) {
      this.messagePart = "Part Price is required";
      return;
    }

    if (this.autopart.title != null && this.autopart.title != '' && this.autopart.title.length > 255) {
      this.messagePart = "Parts Name is too long";
      return;
    }

    if (this.autopart.description != null && this.autopart.description != '' && this.autopart.description.length > 2000) {
      this.messagePart = "Part Description is too long";
      return;
    }

    if (this.autopart.comments != null && this.autopart.comments != '' && this.autopart.comments.length > 200) {
      this.messagePart = "Part comments is too long";
      return;
    }

    if (this.autopart.description != null && this.autopart.description != '' && this.autopart.description.length < 2) {
      this.messagePart = "Part Description is too short";
      return;
    }

    if (this.autopart.comments != null && this.autopart.comments != '' && this.autopart.comments.length < 2) {
      this.messagePart = "Part comments is too short";
      return;
    }

    if (this.autopart.partNumber != null && this.autopart.partNumber != '' && this.autopart.partNumber.length > 255) {
      this.messagePart = "Parts Number is too long";
      return;
    }
      ...
      if (this.forPurchaseOrder == false) {
        this.autopartService.create(this.autopart).subscribe({
          next: (res) => {
            ...
            this.autopart = res;
            this.messagePart = "Successfully Created";

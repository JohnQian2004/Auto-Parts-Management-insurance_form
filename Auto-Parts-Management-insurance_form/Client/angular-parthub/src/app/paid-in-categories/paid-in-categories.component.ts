...
@Component({
  selector: 'app-paid-in-categories',
  templateUrl: './paid-in-categories.component.html',
  styleUrls: ['./paid-in-categories.component.css']
})
export class PaidInCategoriesComponent implements OnInit, AfterViewInit {
  ...
  createPaymentType(paymentType: PaymentType): void {
    ...
    if (paymentType.name == null || paymentType.name == '') {
      this.messagePaymentType = "Paid In Category Name is required";
      return;
    }

    if (paymentType.name != null && paymentType.name != '' && paymentType.name.length < 2) {

      this.messagePaymentType = "Paid In Category Name is too short";
      return;
    }


    if (paymentType.name != null && paymentType.name != '' && paymentType.name.length > 255) {

      this.messagePaymentType = "Paid In Category Name is too long";
      return;
    }

    if (paymentType.comments != null && paymentType.comments != '' && paymentType.comments.length > 255) {

      this.messagePaymentType = "Paid In Category Comments are too long";
      return;
    }

    if (paymentType.comments != null && paymentType.comments != '' && paymentType.comments.length < 2) {

      this.messagePaymentType = "Paid In Category Comments are too short";
      return;
    }
    ...
    this.paymentTypeService.createPaymentType(this.currentUser.id, paymentType).subscribe({
      next: result => {
        ...
        if (newEntry == true)
          this.messagePaymentType = "Successfully Created";
        else

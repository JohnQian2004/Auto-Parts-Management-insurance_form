...
export class ReportViewingComponent implements OnInit, OnDestroy {
  ...
  onPaymentStatusFilterChange(event: any): void {
    this.paymentTrackingFilters.paymentStatus = event.target.value;
    console.log('Payment status filter changed to:', this.paymentTrackingFilters.paymentStatus);
    this.applyClientSidePaymentFilters();
  }
  ...
  applyClientSidePaymentFilters(): void {
    ...
    if (this.paymentTrackingFilters.paymentStatus) {
      console.log('🔍 Applying payment status filter:', this.paymentTrackingFilters.paymentStatus);
      filteredVehicles = filteredVehicles.filter(vehicle => {
        const totalEstimate = (vehicle.price || 0) + this.getVehicleTotalSupplements(vehicle);
        const totalPayments = this.getVehicleTotalPayments(vehicle);
        const totalReceipts = this.getVehicleTotalReceipts(vehicle);
        const totalCollected = totalPayments + totalReceipts;
        ...
        if (this.paymentTrackingFilters.paymentStatus === 'paid') {
          // Fully paid: total collected >= estimate
          result = totalCollected >= totalEstimate && totalEstimate > 0;
        } else if (this.paymentTrackingFilters.paymentStatus === 'unpaid') {
          // Unpaid: no payments AND no receipts
          result = totalCollected === 0;
        } else if (this.paymentTrackingFilters.paymentStatus === 'partial') {
          // Partial: has some payments/receipts but less than total estimate
          result = totalCollected > 0 && totalCollected < totalEstimate && totalEstimate > 0;
        }

        // Debug logging for first few vehicles
        if (filteredVehicles.length < 5) {
          console.log(`Vehicle ${vehicle.id}: estimate=$${totalEstimate}, payments=$${totalPayments}, receipts=$${totalReceipts}, collected=$${totalCollected}, filter=${this.paymentTrackingFilters.paymentStatus}, result=${result}`);
        }

        return result;
      });
      console.log(`🎯 Filter result: ${filteredVehicles.length} vehicles match '${this.paymentTrackingFilters.paymentStatus}' status`);
    }

...
import com.xoftex.parthub.repository.Receipt2Repository;
...
public class Receipt2Controller {
  ...
  @Autowired
  Receipt2Repository receipt2Repository;
  ...
  private static final Logger LOG = LoggerFactory.getLogger(Receipt2Controller.class);

  @PostMapping("/{id}")
  @PreAuthorize("hasAnyRole('USER', 'MODERATOR','ADMIN','SHOP', 'RECYCLER')")
  public ResponseEntity<Receipt2> createAndUpdateReceipt2(@PathVariable("id") long id, @RequestBody Receipt2 receiptIn) {
    ...
    if (userOptional.isPresent()) {
      ...
      receipt = this.receipt2Repository.save(receiptIn);
      ...
      if (receiptIn.getComments() != null)
        vehicleHistory.setName("Receipt2 " + receiptIn.getReason() + " " + receiptIn.getComments());
      else
        vehicleHistory.setName("Receipt2 " + receiptIn.getReason());

      vehicleHistory.setUserId(id);
      vehicleHistory.setVehicleId(receiptIn.getVehicleId());

      // vehicleHistory.setName("Receipt2");
  ...
  @PreAuthorize("hasAnyRole('USER', 'MODERATOR','ADMIN','SHOP', 'RECYCLER')")
  public ResponseEntity<List<Receipt2>> updateSequenceNumber(@PathVariable("vehicleId") long vehicleId,
      @RequestBody List<SequenceCarrier> sequenceCarriers) {

    List<Receipt2> receipts = this.receipt2Repository.findByVehicleId(vehicleId);
    ...
    for (Receipt2 receipt : receipts) {
      ...
      for (SequenceCarrier sequenceCarrier : sequenceCarriers) {
        ...
        if (receipt.getId() == sequenceCarrier.getId()) {
          ...
          receipt = this.receipt2Repository.save(receipt);
        }
      }
    }
  ...
  @GetMapping("/{id}")
  @PreAuthorize("hasAnyRole('USER', 'MODERATOR','ADMIN','SHOP', 'RECYCLER')")
  public ResponseEntity<Receipt2> getReceipt2(@PathVariable("id") long id) {
    LOG.info("" + id);
    Optional<Receipt2> receiptOptional = this.receipt2Repository.findById(id);
    Receipt2 receipt = new Receipt2();
  ...
  // @PreAuthorize("hasAnyRole('USER', 'MODERATOR','ADMIN','SHOP', 'RECYCLER')")
  public ResponseEntity<List<Receipt2>> getVehicleReceipt2(@PathVariable("vehicleId") long vehicleId) {
    LOG.info("" + vehicleId);

    List<Receipt2> receipts = this.receipt2Repository.findByVehicleIdOrderByNameAsc(vehicleId);
  ...
  @DeleteMapping("/{id}")
  @PreAuthorize("hasAnyRole('USER', 'MODERATOR','ADMIN','SHOP', 'RECYCLER')")
  public ResponseEntity<?> deleteReceipt2(@PathVariable("id") long id) {
    LOG.info("" + id);
    Optional<Receipt2> receiptOptional = this.receipt2Repository.findById(id);
    Receipt2 receipt = new Receipt2();
    if (receiptOptional.isPresent()) {
      ...
      VehicleHistory vehicleHistory = new VehicleHistory();
      // vehicleHistory.setName("Receipt2");
      vehicleHistory.setName("Receipt2 " + receiptOptional.get().getNotes());
      ...
      this.receipt2Repository.delete(receipt);
  ...
  @PreAuthorize("hasAnyRole('USER', 'MODERATOR','ADMIN','SHOP', 'RECYCLER')")
  public ResponseEntity<?> deleteReceipt2WithUserId(@PathVariable("userId") long userId, @PathVariable("id") long id) {
    LOG.info("" + id);
    Optional<Receipt2> receiptOptional = this.receipt2Repository.findById(id);
    Receipt2 receipt = new Receipt2();
    if (receiptOptional.isPresent()) {
  ...
  @PreAuthorize("hasAnyRole('USER', 'MODERATOR','ADMIN','SHOP', 'RECYCLER')")
  public ResponseEntity<?> deleteReceipt2WithOption(@PathVariable("id") long id) {
    LOG.info("" + id);
    Optional<Receipt2> receiptOptional = this.receipt2Repository.findById(id);
    Receipt2 receipt = new Receipt2();
    if (receiptOptional.isPresent()) {
  ...
  @PreAuthorize("hasAnyRole('USER', 'MODERATOR','ADMIN','SHOP', 'RECYCLER')")
  public ResponseEntity<?> deleteReceipt2WithOptionWithUserId(@PathVariable("userId") long userId,
